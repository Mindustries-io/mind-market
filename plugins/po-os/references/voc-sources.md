...
### How to pull
- **If a local log file exists:** read it via `Read`
- **If the log lives in a cloud drive:** use a connected drive/file MCP server if one is available (discover the exact tool names at runtime — ToolSearch or the tool list); if no such server is connected, ask the user to export or paste the log
...
- "Moved to {competitor}" = competitive gap — feed it into competitor review mining (and `marketing-os:competitive-intel` if installed)
...
- If the user hasn't run interviews, suggest the sample script below (hypothesis-mode)

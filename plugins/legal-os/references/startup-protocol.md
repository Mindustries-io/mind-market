...
- **READ** (config.json, registries, snapshots): use the first location in that order
  where the file exists (for location 1, "env var set" is enough to select it).
- **WRITE** (setup wizard writing config.json; data files): use the first WRITABLE
  location in the same order, creating directories as needed. When both 2 and 3 are
  plausible and neither exists yet, prefer 3 (home) in Claude Code and 2 in sandboxed
